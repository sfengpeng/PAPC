...
def knn_points(
    ...
    if k == 1:
        ...
        knn_dist, knn_ind = torch.topk(distance, k, dim=2, largest=False, sorted=sorted) # 与每个中心点距离最短的K个点的坐标
...
class KNNGrouper(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor, features: torch.Tensor, use_fps=True):
        ...
        batch_offset = batch_offset.reshape(-1, 1, 1) # b
        ...
        nbr_xyz = nbr_xyz - centers.unsqueeze(2)  # [B, G, K, 3] 中心化 
        ...
        group_feats = torch.cat(group_feats, dim=-1) # bsz x group_num x group_size x (fea_dim + coords_dim)
        return dict(
