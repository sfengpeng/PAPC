...
class COSeg(nn.Module):
    ...
    def forward(
        ...
        # calculate query prototype

        # global_query_for_class1 = []
        # global_query_for_class2 = []
        # label_pesudo = []
        # for i, q_feat in enumerate(query_feat):
            
        #     query_pesudo, prototypes = self.get_mask(
        #         query_mask_proposals[i],
        #         q_feat, support_feat, support_y_low_split, query_y_low[i], support_mask_proposals, count=i
        #     )
        #     global_query_for_class1.append(prototypes[0])
        #     global_query_for_class2.append(prototypes[1])
        #     label_pesudo.append(query_pesudo)

        # #label_pesudo = torch.cat(label_pesudo, dim = 0)
        # global_query_for_class1 = torch.cat(global_query_for_class1, dim = 0)
        # global_query_for_class2 = torch.cat(global_query_for_class2, dim = 0)
        ...
        for i, q_feat in enumerate(query_feat):
            ...
            query_pesudo, prototypes = self.get_mask(
                query_mask_proposals[i],
                q_feat, support_feat, support_y_low_split, query_y_low[i], support_mask_proposals, i
            )
            ...
            correlations = F.cosine_similarity(
                ...
                new_sparse_embeddings[None, :, :],  # 1, (N_way+1)*N_pt, C
                dim=2,
            )  # N_q, (N_way+1)*N_pt
        ...
        return query_y_low.int(), loss, query_y_low, query_x_low, final_pred
    ...
    def get_mask(self, query_proposals, feat_q, feat_s, label_s, label_y, support_proposals, count = None):
        ...
        # return_prototypes = [sup_feat[sup_label.bool()].mean(dim = 0, keepdim=True) 
        #                      for (sup_feat, sup_label) in zip(feat_s, label_s)]

        return_prototypes = [torch.zeros(size=(1,feat_q.shape[1])).cuda()] * 2
        pred_for_class1 = []
        pred_for_class2 = []
        score_for_class1 = []
        score_for_class2 = []
        ...
        for proto, proposal in zip(prototype_q, new_proposals):
            ...
            score_for_class2.append(score)

        if len(pred_for_class1) == 0:
            return torch.stack([query_pesudo_label.int()] * 2, dim = 0), return_prototypes
        
        pred1 = torch.stack(pred_for_class1, dim = 0)
        pred2 = torch.stack(pred_for_class2, dim = 0)
        score1 = torch.stack(score_for_class1, dim = 0)
        score2 = torch.stack(score_for_class2, dim = 0)

        pred1_logits,_ = torch.max(pred1, dim = 0) # N_q
        pred2_logits,_ = torch.max(pred2, dim = 0) # N_q
        score1_m = torch.mean(score_for_class1)
        score2_m = torch.mean(score_for_class2)

        pred1_logits = (pred1_logits - pred1_logits.min()) / (pred1_logits.max() - pred1_logits.min() + 1e-8) * score1_m
        pred2_logits = (pred2_logits - pred2_logits.min()) / (pred2_logits.max() - pred2_logits.min() + 1e-8) * score2_m


        if not torch.equal(pred1_logits, torch.zeros_like(pred1_logits).cuda()):
            return_prototypes[0] = feat_q[pred1_logits.bool()].mean(dim = 0, keepdim = True)
        
        if not torch.equal(pred2_logits, torch.zeros_like(pred2_logits).cuda()):
            return_prototypes[1] = feat_q[pred2_logits.bool()].mean(dim = 0, keepdim = True)
        
        pred2_logits[pred2_logits == 1] = 2
        return torch.stack([pred1_logits.int(), pred2_logits.int()], dim = 0), return_prototypes
