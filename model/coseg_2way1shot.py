...
class COSeg(nn.Module):
    ...
    def forward(
        ...
        for i, q_feat in enumerate(query_feat):
            ...
            query_pesudo, prototypes, flag, pred_logits1, pred_logits2 = self.get_mask(
    ...
    def get_mask(self, query_proposals, feat_q, feat_s, label_s, label_y, support_proposals, count = None, sample_class = None):
        ...
        # return_prototypes2 = [sup_feat[sup_label.bool()].mean(dim = 0, keepdim=True) 
        ...
        # response = response.softmax(dim = 1)

        # print("query label", torch.unique(label_y))
        # print("for class1 max response", torch.mean(response[:,0]))
        # print("for class2 max response", torch.mean(response[:,1]))
        ...
        if len(pred_for_class1) == 0:
            return query_pesudo_label.int(), return_prototypes, True, False, False
        ...
        return query_pesudo_label.int(), return_prototypes, pred1_logits, pred2_logits
