...
def get_indice_pairs(
    ...
    mask_mat = mask.unsqueeze(-1) & mask.unsqueeze(
        -2
    )  # [n, k, k] valid attention range A & A^T, Aij = Aji = True == 原矩阵第i和第j列都为True
    ...
    downsample_mask[downsample_idx.long()] = True # 哪些位置被选中为了FPS的中心点。
...
class BasicLayer(nn.Module):
    ...
    def forward(self, feats, xyz, offset, gt, query_base_y):
        ...
        ) # pav_map: 是一个矩阵 [N X K] ,每行代表簇号，每列表示簇号当中的点的编号  counts: 每个簇有多少个点。
        ...
        # get new_offset after fps for sparse window sampling， 就是将原来的offest除以self.downsample_scale
        ...
        new_offset = torch.tensor(data=new_offset, dtype=int, device='cuda')

        downsample_idx = pointops.furthestsampling(
            xyz, offset.int(), new_offset.int()
        )  # [N/8,] xyz表示需要采样的点云集合，offest表示每个点云中点的数量，new_offest表示采样完毕后每个点云中点的数量。返回下标
        ...
        )  # 双重采样来对齐边缘，有的体素可能在多个不同语义对象的边缘，施加小偏移之后，可能得到更加精确的结果。
...
class Stratified(nn.Module):
    ...
    def forward(
        ...
        Params:
            feats:N,6 xyz: N,3
            offset: 每个batch中点的数量 batch: N (0..0, 1..1,...,b) 每个点属于哪一个batch(shot)
