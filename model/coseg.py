...
class COSeg(nn.Module):
    ...
    def forward(
        ...
        if not self.training:
            query_proposals = query_proposals[0]
        if query_proposals is not None:
            query_mask_proposals = query_proposals.cuda(non_blocking=True) # 
            if query_mask_proposals.shape[0] == 0:
                query_mask_proposals = torch.zeros(size=(1, query_x.shape[0]), device= query_x.device)
        m2_dict = {tuple(row.tolist()): idx for idx, row in enumerate(query_x[:,:3])}
        indices_list = [m2_dict[tuple(mask.tolist())] for mask in query_x_low]
        indices_to_keep_tensor = torch.tensor(indices_list, dtype=torch.long).cuda(non_blocking=True)
        query_mask_proposals = query_mask_proposals[:, indices_to_keep_tensor]
        ...
        for i, q_feat in enumerate(query_feat):
            ...
            query_logits = self.get_mask(query_mask_proposals, 
                                               q_feat, support_feat, support_y_low_split, query_y_low, support_mask_proposals)

            
            norm_logits = (query_logits - query_logits.min()) / (query_logits.max() - query_logits.min() + 1e-8)

            norm_logits[norm_logits >= 0.9] = 1
            norm_logits[norm_logits < 0.9] = 0

            pp = norm_logits.int()
           
            if torch.equal(norm_logits, torch.zeros_like(norm_logits)):
                # in this situation, global_query == mean(support_global_prototypes)
                sup_prototypes = []
                for sup_feat, sup_label in zip(support_feat, support_y_low_split):
                    if torch.sum(sup_label) == 0:
                        continue
                    sup_prototypes.append(sup_feat[sup_label.bool()].mean(dim = 0))
                
                sup_global_prototypes = torch.stack(sup_prototypes, dim = 0)
                global_query = sup_global_prototypes.mean(dim = 0, keepdim = True)
            else:
                global_query = q_feat[norm_logits.bool()].mean(dim = 0, keepdim=True)
                
            seq_len, dim = global_query.shape

            mb = sparse_embeddings[100:, :].clone() 

            qry = mb.reshape(1, 100, 4, 48).type(torch.half)
            key = global_query.reshape(1, seq_len, 4, dim // 4).type(torch.half)
            value = global_query.reshape(1, seq_len, 4, dim // 4).type(torch.half)
            
            for _ in range(self.num_clustering):
                qry = qry + flash_attn_func(qry, key, value)

            mb = qry.reshape(100, 192).type(torch.float)
            sparse_embeddings[100:, :] = mb
        ...
        return pp.int(), loss, query_y_low, query_x_low, final_pred
    ...
    def get_mask(self, query_proposals, feat_q, feat_s, label_s, label_y, support_proposals):
        ...
        # Second, Concat All Support prototypes and scores 

        sup_protos = torch.cat(sup_proto_list, dim = 0)
        sup_scores = torch.cat(sup_proto_score_list, dim = 0)
         
        # Then, calculate query prototypes

        prototype_q = []
        for proposal in query_proposals:
            if torch.sum(proposal) == 0:
                continue
            proto = feat_q[proposal.bool()].mean(dim = 0) # 1,C
            prototype_q.append(proto)

        if len(prototype_q) == 0:
            prototype_q = torch.zeros((1, sup_protos.shape[1])).cuda()
        else:
        ...
        pred = []
        for proto, proposal in zip(prototype_q, query_proposals):
            #proto = feat_q[proposal.bool()].mean(dim = 0, keepdim = True) # 1,C
            score = F.cosine_similarity(sup_protos, proto.unsqueeze(0), dim = 1) # N 
            v = (score * sup_scores)
            score = v.mean() * v.max()
            pred.append(score * proposal)

        if len(pred) == 0:
            pred = torch.zeros((1, feat_q.shape[0])).cuda()
        else:
            pred = torch.stack(pred, dim = 0) # N x N_q, N个掩码, N_q个特征

        pred, index = torch.max(pred, dim = 0) # N_q
